	...
	$.extend( AFCH, {
		...
		Page: function ( name ) {
			...
			this.getCategories = function ( useApi, includeCategoryLinks ) {
				...
				this._revisionApiRequest( true ).done( () => {
					let catRegex = new RegExp( '\\[\\[' + ( includeCategoryLinks ? ':?' : '' ) + 'หมวดหมู่:(.*?)\\s*\\]\\]', 'gi' ),
		...
		actions: {
			...
			logAfc: function ( options ) {
				...
				logPage.getText().done( ( logText ) => {
					...
					const byUser = ' โดย [[ผู้ใช้:' + options.submitter + '|]]';
					const sig = ' ~~~~~\n';
		...
		addTalkPageBanners: function ( wikicode, newAssessment, revId, isBiography, newWikiProjects, lifeStatus, subjectName ) {
			...
			const bannerShellTemplates = 'WikiProject banner shell|WikiProjectBanners|WikiProject Banners|WPB|WPBS|WikiProject cooperation shell|Wikiprojectbannershell|WikiProject Banner Shell|Wpb|WPBannerShell|Wpbs|Wikiprojectbanners|WP Banner Shell|WP banner shell|Bannershell|Wikiproject banner shell|WIkiProjectBanner Shell|WikiProjectBannerShell|WikiProject BannerShell|Coopshell|WikiprojectBannerShell|WikiProject Shell|Scope shell|Project shell|WikiProject shell|WikiProject banner|Wpbannershell|Multiple wikiprojects|Wikiproject banner holder|Project banner holder|WikiProject banner shell\\/test1|Article assessment|WikiProject bannershell|รวมป้ายโครงการ';
			...
			// THWIKI - We didn't use class parameter in WikiProject banner at the moment
